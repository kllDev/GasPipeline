package com.kll.gasPipeline.geo.geotools;


import com.kll.gasPipeline.geo.IO.StringTokenReader;
import com.kll.gasPipeline.geo.pojos.ShpDatas;
import com.kll.gasPipeline.geo.pojos.ShpInfo;
import com.kll.gasPipeline.geo.result.ResponseMessage;
import com.kll.gasPipeline.geo.result.ResponseResult;
...
public class ShpTools {
     ...
     * @param filePath 文件路径
     */
    public static ShpDatas readShpByPath(String filePath, Integer limit, Charset readCharset) throws Exception {
        // 一个数据存储实现，允许从Shapefiles读取和写入
        ShapefileDataStore shpDataStore = new ShapefileDataStore(new File(filePath).toURI().toURL());
        // 设置编码【防止中文乱码】
//        shpDataStore.setCharset(StandardCharsets.ISO_8859_1);
        shpDataStore.setCharset(readCharset);
...
//            System.out.println("\n============================ 序号："+stop+"\n");
    ...
    private static void multiThread(FeatureIterator<SimpleFeature> iterator, ShpDatas shpDatas) {
        ExecutorService pool = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors() - 1);
        while (iterator.hasNext()) {
            SimpleFeature next = iterator.next();
            pool.execute(() -> {
                buildShpDatas(next, shpDatas);
            });
        }
    }
     ...
     * @param shpDatas shp封装的数据集
     */
    private static void buildShpDatas(SimpleFeature feature, ShpDatas shpDatas) {
        // 拿到一个特征
        // 取出特征里面的属性集合
        Collection<Property> p = feature.getProperties();
        // 遍历属性集合
...
//            System.out.println("\n============================ 序号："+stop+"\n");
    ...
    public static void writeShpByGeom(String filePath, Geometry geometry) throws Exception {
        ...
        System.out.println("2========= 写入【" + geometry.getGeometryType() + "】成功 ！=========");
    ...
    public static ResponseResult writeShpByGeom(ShpInfo shpInfo) throws Exception {
        ...
        System.out.println("1========= 写入【" + geometry.getGeometryType() + "】成功 ！=========");
...
//        tBuilder.setCRS(DefaultGeographicCRS.);
